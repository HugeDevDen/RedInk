...
def create_config_blueprint():
    ...
    def get_config():
        ...
        try:
            # 读取图片生成配置
            image_config = _read_config(
                IMAGE_CONFIG_PATH, {"active_provider": "google_genai", "providers": {}}
            )

            # 读取文本生成配置
            text_config = _read_config(
                TEXT_CONFIG_PATH, {"active_provider": "google_gemini", "providers": {}}
            )
    ...
    if "providers" in new_data:
        ...
        for name, new_provider_config in new_providers.items():
            ...
            if new_provider_config.get("api_key") in [True, False, "", None]:
                if name in existing_providers and existing_providers[name].get(
                    "api_key"
                ):
...
def _test_google_genai(config: dict) -> dict:
    ...
    if config.get("base_url"):
        client = genai.Client(
            ...
            http_options={"base_url": config["base_url"], "api_version": "v1beta"},
            vertexai=False,
        )
        # 测试列出模型
        try:
...
def _test_google_gemini(config: dict, test_prompt: str) -> dict:
    ...
    if config.get("base_url"):
        ...
        client = genai.Client(api_key=config["api_key"], vertexai=True)
...
def _test_openai_compatible(config: dict, test_prompt: str) -> dict:
    ...
    base_url = (
        config["base_url"].rstrip("/").rstrip("/v1")
        if config.get("base_url")
...
def _test_image_api(config: dict) -> dict:
    ...
    response = requests.get(
        url, headers={"Authorization": f"Bearer {config['api_key']}"}, timeout=30
    )
...
def _check_response(result_text: str) -> dict:
    ...
    if "你好" in result_text and "红墨" in result_text:
        return {"success": True, "message": f"连接成功！响应: {result_text[:100]}"}
    else:
        return {
            "success": True,
